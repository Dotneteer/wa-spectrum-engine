...
describe("N: Indexed bit ops 00-3f (iy)", () => {
  before(async () => {
  ...
  for (let q = 0; q <= 7; q++) {
    ...
    it(`0${q.toString(16)}: rlc (iy+D)${
      ...
      let m = testMachine.memory;
      s.iy = 0x1000;
      m[s.iy - 256 + OFFS] = 0x08;
      s.f &= 0xfe;

      s = testMachine.run(s, m);
      m = testMachine.memory;

      expect(m[s.iy - 256 + OFFS]).toBe(0x10);
      if (q !== 6) {
        expect(getReg8(s, q)).toBe(m[s.iy - 256 + OFFS]);
      }
      expect(s.f & FlagsSetMask.S).toBeFalsy();
      expect(s.f & FlagsSetMask.Z).toBeFalsy();
      expect(s.f & FlagsSetMask.H).toBeFalsy();
  ...
  for (let q = 0; q <= 7; q++) {
    ...
    it(`0${q.toString(16)}: rrc (iy+D)${
      ...
      let m = testMachine.memory;
      s.iy = 0x1000;
      m[s.iy - 256 + OFFS] = 0x85;
      s.f &= 0xfe;

      s = testMachine.run(s, m);
      m = testMachine.memory;

      expect(m[s.iy - 256 + OFFS]).toBe(0xc2);
      if (q !== 6) {
        expect(getReg8(s, q)).toBe(m[s.iy - 256 + OFFS]);
      }
      expect(s.f & FlagsSetMask.S).toBeTruthy();
  ...
  for (let q = 0; q <= 7; q++) {
    ...
    it(`0${q.toString(16)}: rl (iy+D)${
      ...
      let m = testMachine.memory;
      s.iy = 0x1000;
      m[s.iy - 256 + OFFS] = 0x84;
      s.f &= 0xfe;

      s = testMachine.run(s, m);
      m = testMachine.memory;

      expect(m[s.iy - 256 + OFFS]).toBe(0x08);
      if (q !== 6) {
        expect(getReg8(s, q)).toBe(m[s.iy - 256 + OFFS]);
      }
      expect(s.f & FlagsSetMask.S).toBeFalsy();
  ...
  for (let q = 0; q <= 7; q++) {
    ...
    it(`0${q.toString(16)}: rr (iy+D)${
      ...
      expect(m[s.iy - 256 + OFFS]).toBe(0x42);
      if (q !== 6) {
        expect(getReg8(s, q)).toBe(m[s.iy - 256 + OFFS]);
      }
      expect(s.f & FlagsSetMask.S).toBeFalsy();
  ...
  for (let q = 0; q <= 7; q++) {
    ...
    it(`0${q.toString(16)}: sll (iy+D)${
      ...
      expect(m[s.iy - 256 + OFFS]).toBe(0x11);
      if (q !== 6) {
        expect(getReg8(s, q)).toBe(m[s.iy - 256 + OFFS]);
      }
      expect(s.f & FlagsSetMask.S).toBeFalsy();
